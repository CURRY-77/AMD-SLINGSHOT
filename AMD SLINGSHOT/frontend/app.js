...
const app = {
    ...
    async loadDashboardStats() {
        try {
            ...
            data.history.forEach(item => {
                if (item.type === 'URL') urls++;
                if (item.type === 'Email') emails++;
            ...
            tbody.innerHTML = data.history.map(item => `
                <tr>
                    ...
                    <td><span class="badge ${item.type === 'URL' ? 'badge-info' : 'badge-low'}">${item.type}</span></td>
    ...
    async scanNetwork() {
        ...
        try {
            const response = await fetch(`${API_BASE}/monitor/network`, { method: 'POST' });
            const data = await response.json();
    ...
    async viewHistoryDetails(id) {
        try {
            ...
            if (item) {
                // Formatting JSON beautifully for the modal
                document.getElementById('modal-title').textContent = `${item.type} Scan: ${item.target}`;
                document.getElementById('modal-content').textContent = JSON.stringify(item.details, null, 2);
    ...
    async pollPacketMonitor() {
        try {
            const response = await fetch(`${API_BASE}/monitor/packets`);
            const data = await response.json();
            
            // Update Totals
            document.getElementById('pm-total-sent').textContent = data.total_packets_sent.toLocaleString();
            document.getElementById('pm-total-recv').textContent = data.total_packets_recv.toLocaleString();

            // Update Chart
            const now = new Date().toLocaleTimeString([], {minute:'2-digit', second:'2-digit'});
            this.chartDataX.push(now);
            this.chartDataY.push(data.packets_per_second);
            ...
            if (data.anomalies.length > 0) {
                alertsContainer.innerHTML = data.anomalies.map(a => `<div class="alert badge-critical mt-2">${a}</div>`).join('');
            } else {
